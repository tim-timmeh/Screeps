...
MissionDefender.prototype.initMiss = function () { // Initialize / build objects required
  if (!this.targetAttack) {
    ...
    if (enemyCreeps.length && (enemyCreeps[0].owner.username != 'Invader' && enemyCreeps[0].owner.username != 'Sanvicta' /*Truce for transit 26.2.24*/) || this.room.controller.level < 3 || enemyCreeps.length > 1) { // Check defender for truces also
