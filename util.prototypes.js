...
Creep.prototype.doFillContainer = function (targetC) {
  if (targetC && Object.keys(targetC).length) {
    if (this.transfer(targetC, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
      this.moveToModule(targetC);
    } else {
      this.giveWay()
    }
    return;
